...
<footer class="bg-(--bg-secondary) border-t border-(--border-color)">
	<div class="container-custom py-12">
		<div class="grid md:grid-cols-3 gap-8 mb-8">
			<!-- Brand -->
			<div class="space-y-4">
				<a href="#hero" class="text-2xl font-display font-bold gradient-text"> Henry Zeng </a>
				<p class="text-(--text-secondary) text-sm leading-relaxed max-w-xs">
					Full-Stack Web Developer based in Chicago, Illinois. Building elegant, performant, and
			...
			<div>
				...
				<nav class="space-y-2" aria-label="Footer navigation">
					{#each quickLinks as link}
						<a
							href={link.href}
							class="block text-(--text-secondary) hover:text-primary-500 transition-colors text-sm"
						>
							{link.label}
						</a>
					{/each}
		...
		<!-- Bottom Bar -->
		<div
			class="pt-8 border-t border-(--border-color) flex flex-col md:flex-row justify-between items-center gap-4"
		>
			<p class="text-sm text-(--text-tertiary)">
				© {currentYear} Henry Zeng. All rights reserved.
			</p>
			<p class="text-sm text-(--text-tertiary)">
				Built with
				<span class="text-primary-500">SvelteKit</span>,
				<span class="text-primary-500">TailwindCSS</span>, and
				<span class="text-primary-500">GSAP</span>
			</p>
		</div>
	</div>
</footer>
