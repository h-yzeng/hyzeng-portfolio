...
>
	<div class="container-custom">
		...
		<div class="text-center mb-16">
			...
			<p class="mt-6 text-lg text-[var(--text-secondary)] max-w-2xl mx-auto">
		...
		<div class="skills-grid grid md:grid-cols-2 lg:grid-cols-3 gap-8">
			{#each skillCategories as category}
				<div
					class="skill-category bg-[var(--bg-secondary)] rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow duration-300"
				>
					<!-- Category Header -->
						...
						{#each category.skills as skill}
							<div class="skill-item">
								<div class="flex justify-between items-center mb-2">
									...
									<span class="text-sm text-[var(--text-tertiary)]">{skill.level}%</span>
								</div>
		...
		<div class="mt-16 text-center">
			<h3 class="text-xl font-display font-semibold text-[var(--text-primary)] mb-6">
			...
			<div class="flex flex-wrap justify-center gap-3">
				{#each ['REST APIs', 'GraphQL', 'Firebase', 'PostgreSQL', 'Docker', 'CI/CD', 'Responsive Design', 'Accessibility', 'Performance Optimization', 'SEO'] as tag}
					<span
						class="px-4 py-2 bg-[var(--bg-secondary)] text-[var(--text-secondary)] rounded-full text-sm font-medium hover:bg-primary-500/10 hover:text-primary-500 transition-colors duration-300"
					>
						{tag}
					</span>
				{/each}
